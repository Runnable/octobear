...
module.exports = class Parser {
  ...
  /**
   * Check if pass is github.com URL
   *
   * @param {String}          path
   * @return {Boolean}    true if path is github URL
   */
  static isGitHubUrl (path) {
    return path.indexOf('github.com') > -1
  }
  ...
  static buildDockerfilePathParser (opts) {
    ...
    if (Parser.isGitHubUrl(buildPath)) {
   ...
   */
  static buildRemoveCodeParser (opts) {
    const build = opts.build
    const warnings = opts.warnings
    if (!build || build.disabled) return null
    ...
    if (Parser.isGitHubUrl(buildPath)) {
      const githubUrlTokens = buildPath.split('github.com')
