...
module.exports = class Parser {
  ...
  static dockerBuildParser (opts) {
    ...
    if (dockerContextPath.substr(dockerContextPath.length - 4) === '.git') {
      dockerContextPath = dockerContextPath.substr(0, dockerContextPath.length - 4)
    }
    // if path has SCM domain name than we ignore it when creating path or Docker file
    const dockerFilePath = Parser.isScmUrl(scmDomain, dockerContextPath)
      ? path.resolve('/', dockerFileName)
      : path.resolve('/', dockerContextPath, dockerFileName)
    const dockerBuildContext = Parser.isScmUrl(scmDomain, dockerContextPath)
      ? undefined : dockerContextPath
    return {
      dockerFilePath,
      dockerBuildContext
    }
