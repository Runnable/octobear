...
module.exports = class Parser {
  ...
  /**
   * Parser for `buildDockerContext`
   *
   * @param {Object}          opts
   * @param {String|Object}   opts.build
   * @param {String|Object}   opts.scmDomain
   * @param {Object}          opts.warnings
   * @return {Stribg|Null}    Docker build context path
   */
  static buildDockerContextParser (opts) {
    const build = opts.build
    const warnings = opts.warnings
    const scmDomain = opts.scmDomain
    if (!build || build.disabled) return null
    ...
    const buildContextPath = build.context
    if (!buildContextPath || !Parser.isScmUrl(scmDomain, buildContextPath)) {
      return
    }
