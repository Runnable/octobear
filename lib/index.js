...
module.exports = class DockerComposeParser {
  ...
  static parse (opts) {
    ...
    .tap(services => {
      if (!opts.skipMissingMainCheck) {
        DockerComposeParser._checkIfMissingMain(services, repositoryName)
      }
    })
  ...
  static _mergeServices (allServices, opts) {
    ...
    const services = sortedServices.reduce(
      (servs, service) => {
        ...
        if (existingServiceIndex > -1) {
          ...
          // isMain should be set from parent
          if (existingService.metadata.isMain) {
            mergedService.metadata.isMain = existingService.metadata.isMain
          }
    ...
    DockerComposeParser._checkIfMissingMainToArray(services, opts)
    const envFiles = DockerComposeParser._getAllENVFiles(services)
    return { results: services, envFiles }
  }
   ...
   * @private
   */
  static _checkIfMissingMainToArray (services, opts) {
    const repositoryName = opts.repositoryName
    const userContentDomain = opts.userContentDomain
    const ownerUsername = opts.ownerUsername
    const mainService = services.find(service => service.metadata.isMain)
    if (!mainService) {
      const newMainService = {
        ...
        files: Parser.filesParser({ image: 'busybox' }),
        instance: {
          name: repositoryName,
          env: []
        }
