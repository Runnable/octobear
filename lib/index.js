...
=======
  ...
  static _serviceParser (opts) {
    ...
    .spread((rawService, warnings) => {
      const res = Promise.props({
        ...
        files: Parser.filesParser({ image: rawService.image, warnings, dockerComposeFilePath }),
  ...
  static getMainName (services) {
    const mainService = Object.keys(services).find(k => services[k].build)
    if (mainService) return mainService._name
    return null
  }
}
