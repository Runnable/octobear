...
module.exports = class DockerComposeParser {
  ...
  static _convertToArrayAndAddMetadata (services, dockerComposeFileDirectoryPath) {
    ...
    return Object.keys(services).map(serviceKey => {
      const links = Parser.linksParser({ links: services[serviceKey].links })
      const dependsOn = services[serviceKey].depends_on || []
      const envFiles = Parser.envFileParser({ env_file: services[serviceKey].env_file, dockerComposeFileDirectoryPath })
      return {
        metadata: {
          name: serviceKey,
  ...
  static _populateEnvsWithHostFromServices (services) {
    ...
    return services.map(service => {
      service.instance.aliases = Parser.aliasInstanceNameReplacer({ aliases: service.instance._aliases, servicesNameMap })
      delete service.instance._aliases
  ...
  static _mapRawServiceToService (service, rawService, repositoryName, warnings) {
    return Promise.props({
      ...
      instance: Promise.props({
        ...
        _aliases: Parser.aliasParser({ links: rawService.links }) // Partial parsing
      })
    })
  }
