...
module.exports = class DockerComposeParser {
  ...
  /**
   * Parse a list if docker-compose.yml file sinto an array of services.
   *
   * @param {Object} opts
   * @param {String} opts.repositoryName - Name of repository in GH or other service
   * @param {String} opts.userContentDomain - User content domain for API environment. Used for creating hostnames.
   ...
   * @return {Promise}
   * @resolves {Object[]}        Array with objects for all services
   */
   ...
   * @returns {Object[]} array of services where `inheritance` was resolved by merging
   */
  ...
  static findAllEnvFiles (files) {
    return Promise.map(files, (file) => {
      const dockerComposeFileString = file.dockerComposeFileString
      const dockerComposeFilePath = file.dockerComposeFilePath || './docker-compose.yml'
      const dockerComposeFileDirectoryPath = path.dirname(dockerComposeFilePath)
