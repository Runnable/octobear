...
module.exports = class DockerComposeParser {
  ...
  /**
   * Parse a list if docker-compose.yml files into an array of services.
   *
   * @param {Object} opts
   * @param {String} opts.repositoryName - Name of repository in GH or other service
   * @param {String} opts.userContentDomain - User content domain for API environment. Used for creating hostnames.
  ...
  static parseAndMergeMultiple (opts, files) {
    console.log('PARSE AND MERGE MULTIPLE:', files)
    ...
    .then((parsedResults) => {
      const allServices = parsedResults.reduce(
        (servs, item) => {
          servs = servs.concat(item.results)
          return servs
        },
        [])
      return DockerComposeParser._mergeServices(allServices)
    })
  }
   ...
   * @resolves {Array<Object>}   result.results  - Array with objects for all services where `inheritance` was resolved by merging
   * @resolves {Array<String>}   result.envFiles - Array with all required ENV files
   */
  static _mergeServices (allServices) {
    console.log('MERGE MULTIPLE', allServices)
    ...
    const services = sortedServices.reduce(
      (servs, service) => {
        console.log('MERGE REDUCE', service)
        ...
        console.log('MERGE REDUCE INDEX', existingServiceIndex)
        if (existingServiceIndex > -1) {
          const existingService = servs[existingServiceIndex]
          console.log('MERGING START', existingService, service)
          ...
          console.log('MERGING ENVS', existingService.instance.env, service.instance.env, mergedService.instance.env)
          console.log('MERGING END')
